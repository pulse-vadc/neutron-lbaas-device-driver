...
class PulseDeviceDriverV2(vTMDeviceDriverCommon):
    ...
    def update_loadbalancer(self, lb, old):
        ...
        Can also update the bandwidth allocation of the vTM instance.
        """
        deployment_model = self._get_setting(
            lb.tenant_id, "lbaas_settings", "deployment_model"
        )
...
#############
    ...
    def update_listener(self, listener, old):
        ...
        deploy_ha_pairs = self._get_setting(
            listener.tenant_id, "lbaas_settings", "deploy_ha_pairs"
        )
        hostname = self._get_hostname(listener.loadbalancer)
        if(deployment_model in ["PER_TENANT", "PER_SUBNET"]
        or deploy_ha_pairs is True):
...
###########
    ...
    @logging_wrapper
    def get_member_health(self, member):
        hostname = self._get_hostname(member.pool.root_loadbalancer)
        vtm = self._get_vtm(hostname)
        status = super(PulseDeviceDriverV2, self).get_member_health(
            member, vtm
        )
        return status
